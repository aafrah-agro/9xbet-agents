...
<div class="card" style="max-width:560px">
    <div class="card-header">
        <h2><i class="fas fa-plus" style="color:var(--primary)"></i> Add Social Link</h2>
    ...
    <form method="POST" action="{{ route('admin.social-links.store') }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="platform" class="form-control"
                   value="{{ old('platform') }}" placeholder="Telegram, Facebook..." required>
            @error('platform')<div class="err">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
            <label>URL *</label>
            <input type="url" name="url" class="form-control"
                   value="{{ old('url') }}" placeholder="https://t.me/..." required>
            @error('url')<div class="err">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
            <label>Font Awesome Icon Class *
                <small style="color:var(--muted)">e.g. fab fa-telegram, fab fa-facebook-f</small>
            </label>
            <input type="text" name="icon" class="form-control"
                   value="{{ old('icon') }}" placeholder="fab fa-telegram" required>
